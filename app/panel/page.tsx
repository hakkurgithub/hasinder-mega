...
export default function MemberDashboard() {
  ...
  const [trades, setTrades] = useState([
    ...
    { id: 'TIB-002', title: 'Insaat Malzemesi / Demir', amount: '120.000 TL', status: 'Onaylandi', detail: 'İstanbul Şantiye Teslim Q12 Nervürlü Demir', date: '28.02.2026' },
    { id: 'TIB-003', title: 'Tekstil / Hammadde', amount: '75.000 TL', status: 'Eslesme Bekliyor', detail: 'Pamuklu Kumaş Ham Bez Alımı', date: '01.03.2026' }
  ]);

  const startMatch = (id) => {
    setTrades(trades.map(t => t.id === id ? {...t, status: 'Eslesme Baslatildi'} : t));
    setSelectedTrade(null);
    alert("TIB Agi: Eslesme talebiniz baskanlik onayina sunuldu!");
  };
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-[#1B365D] p-6 rounded-[2rem] text-white shadow-xl flex flex-col md:flex-row justify-between items-center border-b-4 border-[#D4AF37]">
          <div>
            ...
            <p className="text-[10px] opacity-60 font-mono tracking-widest uppercase">Silicon Campus Secured Node v3.1</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          <div className="lg:col-span-3 space-y-6">
            <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100">
              <h2 className="font-black text-sm uppercase mb-6 border-b pb-4">AKTIF TICARET VE ESLESME MERKEZI</h2>
              
              <div className="overflow-x-auto">
                <table className="w-full text-left text-sm">
                  <thead>
                    <tr className="text-gray-400 uppercase text-[10px] border-b">
                      <th className="pb-3 text-center">Tıkla</th>
                  ...
                  <tbody className="divide-y">
                    {trades.map((trade) => (
                      <tr key={trade.id} onClick={() => setSelectedTrade(trade)} className="hover:bg-blue-50 cursor-pointer transition-all group">
                        <td className="py-4 text-center opacity-0 group-hover:opacity-100 text-blue-500 font-bold">���</td>
          ...
          <div className="space-y-6">
            <div className="bg-[#0A192F] p-6 rounded-[2rem] shadow-xl text-white border-t-4 border-[#D4AF37]">
              ...
              <p className="text-center font-bold text-xs">SAYIN ÜYE</p>
              <div className="pt-4 mt-4 border-t border-white/10 flex justify-between text-[10px]">
                <span>GÜVEN SKORU:</span>
                <span className="text-[#D4AF37] font-black">85/100</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white w-full max-w-md rounded-[2.5rem] p-8 shadow-2xl border-t-8 border-[#D4AF37]">
            <div className="flex justify-between items-start mb-6">
              ...
              <button onClick={() => setSelectedTrade(null)} className="text-gray-400 hover:text-red-500 font-bold">KAPAT</button>
            </div>
            <div className="space-y-4">
              ...
              <div className="flex justify-between p-4 border rounded-2xl">
                <div><p className="text-[10px] text-gray-400 uppercase">Hacim</p><p className="font-black text-[#D4AF37]">{selectedTrade.amount}</p></div>
                <div><p className="text-[10px] text-gray-400 uppercase">Tarih</p><p className="font-bold text-xs">{selectedTrade.date}</p></div>
              </div>
