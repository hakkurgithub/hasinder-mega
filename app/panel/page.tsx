...
const S_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...'; // kendi anon key'in
    ...
    <div className="min-h-screen bg-gray-100 dark:bg-[#1B365D] font-sans p-10">
      ...
      >
        ...
        <div className="bg-white dark:bg-[#243B6B] p-6 rounded-2xl shadow flex items-center gap-4">
          ...
          <div>
            ...
            <h2 className="text-xl font-bold">{trades.filter(t => t.status === 'AKTIF').length}</h2>
          </div>
        </div>
        <div className="bg-white dark:bg-[#243B6B] p-6 rounded-2xl shadow flex items-center gap-4">
          <BarChart3 className="text-[#D4AF37]" />
      ...
      >
        ...
        <table className="w-full text-left">
          ...
          <tbody>
            {trades.map(trade => (
              <tr key={trade.id} className="border-b hover:bg-gray-50 dark:hover:bg-[#1B365D]/30">
