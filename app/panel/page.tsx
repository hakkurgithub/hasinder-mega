    ...
    <div className="min-h-screen bg-[#F8FAFC] p-4 lg:p-8 text-[#1B365D]">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100">
          <h2 className="font-black text-xs uppercase mb-4">AKTIF TICARET VERILERI</h2>
          {loading ? (
            ...
            <div className="text-center py-10">
              ...
              <p className="text-[10px] mt-2 text-blue-500">Supabase Table Editor uzerinden veri ekleyiniz.</p>
            </div>
          ) : (
                ...
                <tbody className="divide-y">
                  {trades.map((t) => (
                    <tr key={t.id} className="hover:bg-gray-50 border-b">
                      <td className="py-4 font-mono text-blue-600 font-bold">{t.id}</td>
                      <td className="py-4 font-bold">{t.title}</td>
                      <td className="py-4">
                        <span className="bg-blue-50 px-2 py-1 rounded text-[9px] font-black">{t.status}</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
