    ...
    <div className="min-h-screen bg-[#F0F2F5] text-[#1B365D]">
      <div className="flex">
        <aside className="w-64 bg-[#1B365D] min-h-screen p-6 text-white hidden lg:block shadow-2xl">
          <h1 className="text-xl font-black italic text-[#D4AF37] mb-10">TIB AG V6.2</h1>
          <nav className="space-y-4 font-bold text-sm">
            <button onClick={() => setActiveTab('borsa')} className="w-full text-left p-3 rounded-xl hover:bg-white/10">Borsa Live</button>
            <button className="w-full text-left p-3 rounded-xl hover:bg-white/10">My Trades</button>
        ...
        <main className="flex-1 p-4 lg:p-10 space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="bg-white p-6 rounded-3xl shadow-sm border-b-4 border-blue-500">
              <p className="text-[10px] text-gray-400 font-bold uppercase">Total Ads</p>
          ...
          <section className="bg-white rounded-[2rem] shadow-xl overflow-hidden">
            <div className="p-8 border-b border-gray-100"><h2 className="font-black uppercase">Operation Center</h2></div>
              ...
              {loading ? (
                ...
                <table className="w-full text-left">
                  <thead className="text-[10px] uppercase text-gray-400 border-b">
                    <tr><th className="p-4">Category</th><th>Title</th><th>Volume</th><th className="p-4">Status</th></tr>
                  </thead>
                  <tbody className="divide-y">
                    {trades.map((t) => (
                      <tr key={t.id} className="hover:bg-gray-50 text-sm font-bold">
                        <td className="p-4"><span className="bg-blue-100 text-blue-700 px-3 py-1 rounded-full text-[9px]">{t.category || 'GIDA'}</span></td>
                        <td>{t.title}</td>
                        <td className="text-green-700 font-black">{t.amount}</td>
                        <td className="p-4"><span className="bg-gray-100 px-3 py-1 rounded-full text-[9px] uppercase">{t.status}</span></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-[#1B365D]/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-lg rounded-[2.5rem] p-10 shadow-2xl border-t-[10px] border-[#D4AF37]">
            <h2 className="text-xl font-black mb-6 uppercase">Create New Ad</h2>
            <form onSubmit={handleCreate} className="space-y-4">
              <input placeholder="Title" className="p-4 bg-gray-50 rounded-xl w-full font-bold" value={formData.title} onChange={(e) => setFormData({...formData, title: e.target.value})} required />
              <input placeholder="Amount (TL)" className="p-4 bg-gray-50 rounded-xl w-full font-bold" value={formData.amount} onChange={(e) => setFormData({...formData, amount: e.target.value})} required />
              <div className="flex justify-end gap-4 pt-4">
                <button type="button" onClick={() => setIsModalOpen(false)} className="font-bold text-gray-400">CANCEL</button>
                <button type="submit" className="bg-[#1B365D] text-[#D4AF37] px-8 py-3 rounded-xl font-black">PUBLISH</button>
              </div>
            </form>
          </div>
        </div>
